...
function allMarkup() {
  ...
  return [
    ...
    // The prefilled variant carries a second commit button and a pre-selected
    // chip — neither exists in the plain branch.
    renderAddSheet({
      categories: CATS.filter((c) => !c.vault),
      presetId: "coffee",
    }),
    renderIncomeSheet({
      monthLabel: "August",
    ...
    // Category detail — both branches. The populated one carries the txn rows
    // and the FAB; the empty one carries the empty state and its CTA.
    renderCategoryScreen({
      id: "coffee",
      name: "Coffee",
      monthLabel: "August",
    ...
    // A closed month drops the add affordances and gains the explanation.
    renderCategoryScreen({
      id: "food",
      name: "Food",
      monthLabel: "July",
    ...
    // Edit sheet — the chip--active branch and the date field only exist here.
    renderEditSheet({
      id: "a",
      kind: "expense",
      cent: 18000,
